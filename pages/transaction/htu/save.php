<?php 
  ...
  $htu_code                = mysqli_escape_string($config, "htu-".date('Ymd.').$generateCode);
...
if (mysqli_num_rows($querySelectData)) {
  ...
  $insertData = "INSERT INTO tb_master_htu(
            id_htu, htu_code, htu_description, ts_insert, bl_state)
    VALUES ('$generateId', '$htu_code', '$i_htu_description', '$currentDate $currentTime', 'A')";
  $queryInsertData = mysqli_query($config, $insertData);
    ...
    $insertLogActivity = "INSERT INTO log_activity(
        id_log, log_date, log_menu, log_description, log_status, ip_address, user_name, log_os, log_browser)
            VALUES ( '".sha1(generate(10))."', '$currentDate "." $currentTime', 'INSERT', 'Menambahkan Data Cara Pakai ".$i_htu_description."' , 'A', '$ip_address', '$sessionUser', '$os', '$browser')";
    $queryInsertLogActivity = mysqli_query($config, $insertLogActivity);
    if ($queryInsertLogActivity) {
      echo "<script>$.notify('Berhasil Insert Data Cara Pakai ".$i_htu_description."','success');clearInsertForm();enabledInsertForm();$('#listHTU').modal('show');</script>";
    }else{
