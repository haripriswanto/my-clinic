...
if (mysqli_num_rows($querySelectData)) {
  echo "<script>enabledInsertForm();$.notify('Kode Produk ".$i_product_code." Sudah Ada', 'error'); $('#i_product_code').focus();enabledInsertForm();</script>";
}
else{
  ...
  $queryInsertData = mysqli_query($config, $insertData);
    ...
    if ($stockable == '1') {
      ...
      if ($queryInsertLogData) {
        echo "<script>$.notify('Berhasil Insert Data Produk ".$i_product_name."', 'success');clearInsertForm();loadData();closeForm();</script>";
      }else{
